...
void W25Qx_Read_JEDECID(uint32_t *JEDECID)
{
	uint8_t idt[3];
	
	uint8_t cmd[1] = {READ_JEDEC_ID_CMD};
	
	W25Qx_Enable();
	/* Send the read ID command */
	HAL_SPI_Transmit(&hspi1, cmd, sizeof(cmd), W25QXXXX_TIMEOUT_VALUE);	
	/* Reception of the data */
	HAL_SPI_Receive(&hspi1,idt, sizeof(idt), W25QXXXX_TIMEOUT_VALUE);
